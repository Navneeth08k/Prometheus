...
class CompactFinancialAnalysisSystem:
    ...
    def build_meta_model(self):
        ...
        # Normalize signals
        scaler = StandardScaler()
        signal_df_scaled = pd.DataFrame(scaler.fit_transform(signal_df), index=signal_df.index, columns=signal_df.columns)
        ...
        self.meta_feature_importances = dict(zip(signals, self.meta_model.coef_[0]))
        return self
    ...
    def predict_market(self):
        ...
        # Scale latest signals using the same scaler logic
        scaler = StandardScaler()
        latest_signals_scaled = pd.DataFrame(scaler.fit_transform(latest_signals), columns=latest_signals.columns)
        
        meta_pred_prob = self.meta_model.predict_proba(latest_signals_scaled)[:, 1][0]
...
if __name__ == "__main__":
    ...
    prediction, confidence, top_features = system.run_analysis(retrain=True)  # Set retrain=True to force new training
