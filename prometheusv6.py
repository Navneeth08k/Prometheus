...
Final Prediction: Market will go UP (Confidence: 66.91%)

Top Meta-Model Signals:
1. ensemble_signal: 0.3373
2. mean_reversion_20: 0.1520
3. lstm_signal: 0.1401
4. supertrend_cont: 0.1253
5. momentum_21: 0.1179
6. vix_signal: 0.0297
7. yield_curve_signal: 0.0259
8. atr_signal: 0.0230
9. macd_cross: 0.0170
10. obv_signal: 0.0163
11. bb_signal: 0.0111
12. rsi_signal: 0.0044
13. inverse_cramer: 0.0000
...
On more runs, I see that LSTM can have this result:
LSTM:
  Accuracy: 0.0000
  Precision: 0.0000
  Recall: 0.0000
...
This one has a clear jump in accuracy and precision
This is due to changing from daily to monthly probably.
...
class EnhancedFinancialAnalysisSystemV6:
    def __init__(self, symbol='SPY', lookback_years=5):  # Increased to 5 years
        self.symbol = symbol
    ...
    def build_lstm(self):
        ...
        if os.path.exists('lstm_model.h5') and not hasattr(self, 'retrain'):
            ...
            self.lstm.fit(self.X_train_lstm, self.y_train_lstm, epochs=50, batch_size=8,
    ...
    def build_meta_model(self):
        ...
        signal_df = self.indicators[self.symbol][signals].dropna()
        # We specifically align it with the feature_matrix from index[6:] onward, but some signals might exist from day 0.
        # For simplicity, let's just intersect with the full feature_matrix index:
        valid_index = self.feature_matrix.index.intersection(signal_df.index)
        signal_df = signal_df.loc[valid_index]
        target = self.target.loc[valid_index]
